...
public class registerpatientController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        RegisterPatientModel pat = extractPatientModel(req);
        boolean ok = svc.addPatient(pat);

        if (ok) {
            req.setAttribute("success", " Patient have been successfully registered!");
            req.getRequestDispatcher("/WEB-INF/pages/register_patient.jsp")
               .forward(req, res);
        } else {
    ...
    private RegisterPatientModel extractPatientModel(HttpServletRequest r) {
